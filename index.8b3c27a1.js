...
})({"ceBJz":[function(require,module,exports,__globalThis) {
...
//
parcelHelpers.export(exports, "GraphDetails", ()=>GraphDetails);
/**
 * Convert an HTMLTableElement to a CSV string.
 */ parcelHelpers.export(exports, "tableToCSV", ()=>tableToCSV);
/**
...
var _guiAnalysis = require("./gui_analysis");
 ...
 */ function parse(xml) {
    ...
    if (xml.getElementsByTagName((0, _xmlAnalysisJs.Analysis).tagName).length > 0) {
        ...
        remove(aDivID);
        // Create collapsible content.
        let acDiv = (0, _htmlJs.getCollapsibleDiv)(aDivID, aDiv, null, (0, _guiAnalysis.processAnalysis)(xml), (0, _xmlAnalysisJs.Analysis).tagName, boundary1, level0);
    }
}
...
class ScatterPlot {
    constructor(canvas, data, font, graphdetails){
        this.canvas = canvas;
        this.data = data;
     ...
     */ draw(font) {
        ...
        let ymargin = maxTickLabelWidth + th * 4;
        let x0 = ymargin;
        ...
        ctx.textAlign = "center";
        ctx.textBaseline = "top";
        let yLabel = this.graphdetails.yAxisLabel;
        if (iDeciFctr != 0) yLabel += "/1.e" + iDeciFctr.toString();
        ...
        // Draw y-axis ticks.
        let DeciFctr = 10.0 ** iDeciFctr;
        yTickSpacing = (y1 - y0) / (yTicks - 1);
        let diff = (yMax - yMin) / (yTicks - 1);
        for(let i = 0; i < yTicks; i++){
            let yTick = y0 + i * yTickSpacing; // Convert yTick to pixel scale;
            let yValue = Number((yMin + i * diff) / DeciFctr).toPrecision(2);
            ctx.beginPath();
            ctx.moveTo(x0, yTick);
...
},{"big.js":"91nMZ","./app.js":"dPB9w","./xml_conditions.js":"cZv1r","./html.js":"aLPSL","./xml_mesmer.js":"8G2m7","./xml_metadata.js":"5YFPw","./xml_molecule.js":"cg9tc","./util.js":"f0Rnl","./xml.js":"7znDa","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"9mmoE":[function(require,module,exports,__globalThis) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
/**
 * Parses xml to initialise analysis.
 * @param xml The XML document.
 */ parcelHelpers.export(exports, "processAnalysis", ()=>processAnalysis);
var _bigJs = require("big.js");
var _bigJsDefault = parcelHelpers.interopDefault(_bigJs);
var _xmlAnalysis = require("./xml_analysis");
...
function processAnalysis(xml) {
    console.log((0, _xmlAnalysis.Analysis).tagName);
    let aDivID = (0, _app.addRID)((0, _xmlAnalysis.Analysis).tagName, 0);
    let aDiv = (0, _html.createDiv)(aDivID, (0, _app.boundary1));
    let xml_as = xml.getElementsByTagName((0, _xmlAnalysis.Analysis).tagName);
    if (xml_as.length > 0) {
        if (xml_as.length > 1) throw new Error("More than one Analysis element.");
        let a = new (0, _xmlAnalysis.Analysis)((0, _xml.getAttributes)(xml_as[0]));
        (0, _app.mesmer).setAnalysis(a);
        // "me:description".
        let xml_d = xml_as[0].getElementsByTagName((0, _xmlMesmerJs.Description).tagName);
        if (xml_d.length > 0) {
            if (xml_d.length == 1) {
                let s = (0, _xml.getFirstChildNode)(xml_d[0])?.nodeValue ?? "";
                let d = new (0, _xmlMesmerJs.Description)((0, _xml.getAttributes)(xml_d[0]), s);
                let dDiv = (0, _html.createDiv)((0, _app.addRID)(aDivID, (0, _xmlMesmerJs.Description).tagName), (0, _app.level1));
                aDiv.appendChild(dDiv);
                dDiv.appendChild((0, _html.createLabel)(d.tagName + " " + s, (0, _app.boundary1)));
                a.setDescription(d);
        ...
        // me:rateList.
        let xml_rl = xml_as[0].getElementsByTagName((0, _xmlAnalysis.RateList).tagName);
        // Create a new collapsible div for the RateLists.
        let rlDivID = (0, _app.addRID)(aDivID, (0, _xmlAnalysis.RateList).tagName);
        let rlDiv = (0, _html.createDiv)(rlDivID, (0, _app.level1));
        let rlcDiv = (0, _html.getCollapsibleDiv)(rlDivID, aDiv, null, rlDiv, (0, _xmlAnalysis.RateList).tagName + "s", (0, _app.boundary1), (0, _app.level1));
        if (xml_rl.length > 0) for(let i = 0; i < xml_rl.length; i++){
            let rle_attributes = (0, _xml.getAttributes)(xml_rl[i]);
            ...
            let rl = new (0, _xmlAnalysis.RateList)(rle_attributes);
            let t = rle_attributes.get("T");
            rl.setTemperature(new (0, _bigJsDefault.default)(t));
            let conc = rle_attributes.get("conc");
            rl.setConcentration(new (0, _bigJsDefault.default)(conc));
            ...
            let rleDivID = (0, _app.addRID)(rlDivID, i.toString());
            let rleDiv = (0, _html.createDiv)(rleDivID);
            rlDiv.appendChild(rleDiv);
            let rlecDiv = (0, _html.getCollapsibleDiv)(rleDivID, rlDiv, null, rleDiv, labelText, (0, _app.boundary1), (0, _app.level0));
            let keys;
            let values;
            // "me:firstOrderLoss".
            // Create a new collapsible div for the FirstOrderLosses.
            let folDivID = (0, _app.addRID)(rleDivID, (0, _xmlAnalysis.FirstOrderLoss).tagName);
            let folDiv = (0, _html.createDiv)(folDivID);
            rleDiv.appendChild(folDiv);
            let folcDiv = (0, _html.getCollapsibleDiv)(folDivID, rleDiv, null, folDiv, (0, _xmlAnalysis.FirstOrderLoss).tagName, (0, _app.boundary1), (0, _app.level1));
            let xml_fol = xml_rl[i].getElementsByTagName((0, _xmlAnalysis.FirstOrderLoss).tagName);
            let folTable = (0, _html.createTable)(folDivID, (0, _app.boundary1));
            let folTableDiv = (0, _html.createDiv)((0, _app.addRID)(folDivID, (0, _app.s_table)), (0, _app.level1));
            folTableDiv.appendChild(folTable);
            folDiv.appendChild(folTableDiv);
            for(let j = 0; j < xml_fol.length; j++){
                let fol_attributes = (0, _xml.getAttributes)(xml_fol[j]);
                if (j == 0) {
                    // header
                    ...
                    (0, _html.addTableHeaderRow)(folTable, keys);
                }
                ...
                let s = ((0, _xml.getFirstChildNode)(xml_fol[j])?.nodeValue ?? "").trim();
                let fol = new (0, _xmlAnalysis.FirstOrderLoss)(fol_attributes, new (0, _bigJsDefault.default)(s));
                ...
                values.push(s);
                (0, _html.addTableRow)(folTable, values);
            }
            // Insert a save as csv button.
            (0, _app.addSaveAsCSVButton)(()=>(0, _app.tableToCSV)(folTable), folDiv, folTableDiv, "First Order Losses", (0, _app.level1));
            // "me:firstOrderRate".
            // Create a new collapsible div for the FirstOrderRates.
            let forDivID = (0, _app.addRID)(rleDivID, (0, _xmlAnalysis.FirstOrderRate).tagName);
            let forDiv = (0, _html.createDiv)(forDivID);
            rleDiv.appendChild(forDiv);
            let forcDiv = (0, _html.getCollapsibleDiv)(forDivID, rleDiv, null, forDiv, (0, _xmlAnalysis.FirstOrderRate).tagName, (0, _app.boundary1), (0, _app.level1));
            let xml_for = xml_rl[i].getElementsByTagName((0, _xmlAnalysis.FirstOrderRate).tagName);
            let forTable = (0, _html.createTable)(forDivID, (0, _app.boundary1));
            let forTableDiv = (0, _html.createDiv)((0, _app.addRID)(forDivID, (0, _app.s_table)), (0, _app.level1));
            forTableDiv.appendChild(forTable);
            forDiv.appendChild(forTableDiv);
            for(let j = 0; j < xml_for.length; j++){
                let for_attributes = (0, _xml.getAttributes)(xml_for[j]);
                ...
                if (j == 0) {
                    ...
                    keys2.push("k/" + units);
                    (0, _html.addTableHeaderRow)(forTable, keys2);
                }
                ...
                let s = ((0, _xml.getFirstChildNode)(xml_for[j])?.nodeValue ?? "").trim();
                let forate = new (0, _xmlAnalysis.FirstOrderRate)(for_attributes, new (0, _bigJsDefault.default)(s));
                ...
                values.push(s);
                (0, _html.addTableRow)(forTable, values);
            }
            // Insert a save as csv button.
            (0, _app.addSaveAsCSVButton)(()=>(0, _app.tableToCSV)(forTable), forDiv, forTableDiv, "First Order Rates", (0, _app.level1));
            // "me:secondOrderRate".
            // Create a new collapsible div for the SecondOrderRates.
            let sorDivID = (0, _app.addRID)(rleDivID, (0, _xmlAnalysis.SecondOrderRate).tagName);
            let sorDiv = (0, _html.createDiv)(sorDivID);
            rleDiv.appendChild(sorDiv);
            let sorcDiv = (0, _html.getCollapsibleDiv)(sorDivID, rleDiv, null, sorDiv, (0, _xmlAnalysis.SecondOrderRate).tagName, (0, _app.boundary1), (0, _app.level1));
            let xml_sor = xml_rl[i].getElementsByTagName((0, _xmlAnalysis.SecondOrderRate).tagName);
            let sorTable = (0, _html.createTable)(sorDivID, (0, _app.boundary1));
            let sorTableDiv = (0, _html.createDiv)((0, _app.addRID)(sorDivID, (0, _app.s_table)), (0, _app.level1));
            sorTableDiv.appendChild(sorTable);
            sorDiv.appendChild(sorTableDiv);
            for(let j = 0; j < xml_sor.length; j++){
                let sor_attributes = (0, _xml.getAttributes)(xml_sor[j]);
                ...
                if (j == 0) {
                    ...
                    (0, _html.addTableHeaderRow)(sorTable, keys2);
                }
                ...
                let s = ((0, _xml.getFirstChildNode)(xml_sor[j])?.nodeValue ?? "").trim();
                let sorate = new (0, _xmlAnalysis.SecondOrderRate)(sor_attributes, new (0, _bigJsDefault.default)(s));
                ...
                values.push(s);
                (0, _html.addTableRow)(sorTable, values);
            }
            // Insert a save as csv button.
            (0, _app.addSaveAsCSVButton)(()=>(0, _app.tableToCSV)(sorTable), sorDiv, sorTableDiv, "Second Order Rates", (0, _app.level1));
        }
    }
    return aDiv;
}
// "me:eigenvalueList".
function EigenvalueAnalysis(xml_as, aDivID, aDiv, a) {
    // Create a new collapsible div for the EigenvalueLists.
    let elDivID = (0, _app.addRID)(aDivID, (0, _xmlAnalysis.EigenvalueList).tagName);
    let elDiv = (0, _html.createDiv)(elDivID, (0, _app.level1));
    let elcDiv = (0, _html.getCollapsibleDiv)(elDivID, aDiv, null, elDiv, (0, _xmlAnalysis.EigenvalueList).tagName + "s", (0, _app.boundary1), (0, _app.level1));
    let xml_el = xml_as[0].getElementsByTagName((0, _xmlAnalysis.EigenvalueList).tagName);
    if (xml_el.length > 0) for(let i = 0; i < xml_el.length; i++){
        let el_attributes = (0, _xml.getAttributes)(xml_el[i]);
        let el = new (0, _xmlAnalysis.EigenvalueList)(el_attributes);
        let labelText = el.tagName + " " + i.toString() + " " + (0, _util.mapToString)(el_attributes);
        // Create a new collapsible div for the EigenvalueList.
        let eDivID = (0, _app.addRID)(elDiv.id, i.toString());
        let eDiv = (0, _html.createDiv)(elDivID, (0, _app.level1));
        let ecDiv = (0, _html.getCollapsibleDiv)(eDivID, elDiv, null, eDiv, labelText, (0, _app.boundary1), (0, _app.level0));
        //eDiv.appendChild(createLabel(labelText, boundary1));
        a.addEigenvalueList(el);
        // "me:eigenvalue".
        let evs = [];
        let xml_ei = xml_el[i].getElementsByTagName((0, _xmlAnalysis.Eigenvalue).tagName);
        if (xml_ei.length > 0) for(let j = 0; j < xml_ei.length; j++){
            let ev = new (0, _bigJsDefault.default)((0, _xml.getFirstChildNode)(xml_ei[j])?.nodeValue);
            evs.push(ev);
            el.addEigenvalue(new (0, _xmlAnalysis.Eigenvalue)((0, _xml.getAttributes)(xml_ei[j]), ev));
        }
        eDiv.appendChild((0, _html.createLabel)((0, _util.arrayToString)(evs, ", "), (0, _app.boundary1)));
    }
}
// "me:populationList".
function PopulationEvolution(xml_as, aDivID, aDiv, a) {
    // Create a new collapsible div for the PopulationLists.
    let plDivID = (0, _app.addRID)(aDivID, (0, _xmlAnalysis.PopulationList).tagName);
    let plDiv = (0, _html.createDiv)(plDivID, (0, _app.level1));
    let plcDiv = (0, _html.getCollapsibleDiv)(plDivID, aDiv, null, plDiv, (0, _xmlAnalysis.PopulationList).tagName + "s", (0, _app.boundary1), (0, _app.level1));
    let xml_pl = xml_as[0].getElementsByTagName((0, _xmlAnalysis.PopulationList).tagName);
    if (xml_pl.length > 0) // Create a new collapsible div for the PopulationList.
    for(let i = 0; i < xml_pl.length; i++){
        let pl_attributes = (0, _xml.getAttributes)(xml_pl[i]);
        let pl = new (0, _xmlAnalysis.PopulationList)(pl_attributes);
        let labelText = pl.tagName + " " + i.toString() + " " + (0, _util.mapToString)(pl_attributes);
        let plDivID = (0, _app.addRID)(aDiv.id, (0, _xmlAnalysis.PopulationList).tagName, i.toString());
        // Create a new collapsible div for the EigenvalueList.
        let pDivID = (0, _app.addRID)(plDivID, i.toString());
        let pDiv = (0, _html.createDiv)(plDivID, (0, _app.level1));
        let pcDiv = (0, _html.getCollapsibleDiv)(pDivID, plDiv, null, pDiv, labelText, (0, _app.boundary1), (0, _app.level0));
        ...
        let refs = [];
        refs.push("time");
        let xml_pn = xml_pl[i].getElementsByTagName((0, _xmlAnalysis.Population).tagName);
        if (xml_pn.length > 0) for(let j = 0; j < xml_pn.length; j++){
            let pn_attributes = (0, _xml.getAttributes)(xml_pn[j]);
            let population = new (0, _xmlAnalysis.Population)(pn_attributes, []);
            pl.addPopulation(population);
            let t = pn_attributes.get("time") != undefined ? new (0, _bigJsDefault.default)(pn_attributes.get("time")) : (0, _app.big0);
            ...
            let xml_pop = xml_pn[j].getElementsByTagName((0, _xmlAnalysis.Pop).tagName);
            if (xml_pop.length > 0) for(let k = 0; k < xml_pop.length; k++){
                let pop_attributes = (0, _xml.getAttributes)(xml_pop[k]);
                let ref = pop_attributes.get("ref");
                if (j == 0) refs.push(ref);
                let p = new (0, _bigJsDefault.default)((0, _xml.getFirstChildNode)(xml_pop[k])?.nodeValue);
                let pop = new (0, _xmlAnalysis.Pop)(pop_attributes, p);
                population.addPop(pop);
                ref_pop.set(ref, p);
            }
        }
        // Create graph.
        let graphdetails = new (0, _app.GraphDetails)();
        graphdetails.yAxisLabel = "fractional population";
        createGraph(pDiv, pDivID, t_ref_pop, labelText, graphdetails);
        // Create Table.
        let tableDiv = (0, _html.createDiv)((0, _app.addRID)(pDivID, (0, _app.s_table)), (0, _app.boundary1));
        pDiv.appendChild(tableDiv);
        let tab = (0, _html.createTable)((0, _app.addRID)(plDivID, (0, _app.s_table)), (0, _app.boundary1));
        (0, _html.addTableHeaderRow)(tab, refs);
        t_ref_pop.forEach((ref_pop, t)=>{
            ...
            (0, _html.addTableRow)(tab, row);
        });
        tableDiv.appendChild(tab);
        // Insert a save as csv button.
        (0, _app.addSaveAsCSVButton)(()=>(0, _app.tableToCSV)(tab), pDiv, tableDiv, labelText, (0, _app.boundary1));
    }
}
// "me:avEnergyList".
function AverageEnergyEvolution(xml_as, aDivID, aDiv, tagName) {
    // Create a new collapsible div for the PopulationLists.
    let plDivID = (0, _app.addRID)(aDivID, tagName);
    let plDiv = (0, _html.createDiv)(plDivID, (0, _app.level1));
    let plcDiv = (0, _html.getCollapsibleDiv)(plDivID, aDiv, null, plDiv, tagName + "s", (0, _app.boundary1), (0, _app.level1));
    let xml_pl = xml_as[0].getElementsByTagName(tagName);
    let units = "";
    if (xml_pl.length > 0) // Create a new collapsible div for the PopulationList.
    for(let i = 0; i < xml_pl.length; i++){
        let pl_attributes = (0, _xml.getAttributes)(xml_pl[i]);
        ...
        let pl = new (0, _xmlAnalysis.PopulationList)(pl_attributes);
        let labelText = tagName + " " + i.toString() + " " + (0, _util.mapToString)(pl_attributes);
        let plDivID = (0, _app.addRID)(aDiv.id, tagName, i.toString());
        let pDivID = (0, _app.addRID)(plDivID, i.toString());
        let pDiv = (0, _html.createDiv)(plDivID, (0, _app.level1));
        let pcDiv = (0, _html.getCollapsibleDiv)(pDivID, plDiv, null, pDiv, labelText, (0, _app.boundary1), (0, _app.level0));
        // "me:avEnergy".
        // Change to calculate the log of the time when creating the plots.
        let t_ref_pop = new Map();
        let xml_pn = xml_pl[i].getElementsByTagName("me:avEnergy");
        if (xml_pn.length > 0) {
            ...
            for(let j = 0; j < xml_pn.length; j++){
                let pn_attributes = (0, _xml.getAttributes)(xml_pn[j]);
                let species = pn_attributes.get("ref");
                let xml_pop = xml_pn[j].getElementsByTagName("me:Av");
                if (xml_pop.length > 0) for(let k = 0; k < xml_pop.length; k++){
                    let pop_attributes = (0, _xml.getAttributes)(xml_pop[k]);
                    let time = pop_attributes.get("time");
                    let t = pop_attributes.get("time") != undefined ? new (0, _bigJsDefault.default)(pop_attributes.get("time")) : (0, _app.big0);
                    let energy = xml_pop[k].textContent;
                    if (tmp_pop.has(time)) tmp_pop.get(time)?.set(species, (0, _bigJsDefault.default)(energy));
                    else {
                        let ref_pop = new Map();
                        ref_pop.set(species, (0, _bigJsDefault.default)(energy));
                        tmp_pop.set(time, ref_pop);
                    }
                }
            }
            if (tmp_pop.size > 0) for (let key of tmp_pop.keys())t_ref_pop.set((0, _bigJsDefault.default)(key), tmp_pop.get(key));
        }
        // Create graph.
        let graphdetails = new (0, _app.GraphDetails)();
...
// Create graph.
function createGraph(pDiv, pDivID, t_ref_pop, labelText, graphdetails) {
    let graphDiv = (0, _html.createDiv)((0, _app.addRID)(pDivID, s_graph), (0, _app.boundary1));
    pDiv.appendChild(graphDiv);
    let canvas = document.createElement('canvas');
    graphDiv.appendChild(canvas);
    // Create a scatter plot.
    let scatterPlot = new (0, _app.ScatterPlot)(canvas, t_ref_pop, sp_font, graphdetails);
    // Add the scatter plot to the collection.
    scatterPlots.push(scatterPlot);
    //scatterPlot.draw();
    // Add a save to PNG button.
    (0, _app.addSaveAsPNGButton)(canvas, pDiv, graphDiv, labelText);
}

},{"big.js":"91nMZ","./xml_analysis":"1PdDF","./app":"dPB9w","./xml":"7znDa","./html":"aLPSL","./xml_mesmer.js":"8G2m7","./util":"f0Rnl","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"d6DU0":[function(require,module,exports,__globalThis) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
/**
    ...
    */ getHeader() {
     ...
     */ toStringArray() {
        ...
        return [
            t.value.toString(),
            val.value.toString(),
            rev != undefined ? rev.value.toString() : "undef",
            keq != undefined ? keq.value.toString() : "undef"
        ];
    }
    /**
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["ceBJz","dPB9w"], "dPB9w", "parcelRequire94c2")
